...
def _spec(n: int) -> SudokuSpec:
    ...
    if base * base != n:
        raise ValueError(f"Ungültige Größe: {n}. Erlaubt sind nur Quadratzahlen wie 4, 9, 16, ...")
...
def validate_board(board: Board) -> Tuple[bool, str]:
    ...
    if n == 0 or any(len(row) != n for row in board):
        return False, "Board muss quadratisch sein (N x N)."
    ...
    for r in range(n):
        for c in range(n):
            ...
            if not isinstance(v, int):
                return False, f"Ungültiger Wert bei ({r+1},{c+1}): {v} (kein Integer)."
            if v < 0 or v > n:
                return False, f"Ungültiger Wert bei ({r+1},{c+1}): {v} (erlaubt 0..{n})."
            if v == 0:
                continue
            ...
            if (row_used[r] & bit) or (col_used[c] & bit) or (box_used[b] & bit):
                return False, f"Konflikt entdeckt: Zahl {v} doppelt in Zeile/Spalte/Box (Zelle {r+1},{c+1})."
...
def solve_sudoku(board: Board) -> Optional[Board]:
    """
    Solves Sudoku with backtracking + MRV (minimum remaining values) using bitmasks.
    ...
    def dfs() -> bool:
        ...
        if best_idx == -1:
            return True  # solved (no empties left)
